...
export async function getExecutiveKPIs(companyId: string) {
  const [
    ...
    db.stockBalance.count({ where: { companyId, quantity: { lte: db.stockBalance.fields.reorderLevel } } }),
    db.vehicle.count({ where: { companyId, status: { not: "DECOMMISSIONED" } } }),
    db.driver.count({ where: { companyId, status: "ACTIVE" } }),
    db.tripOrder.count({ where: { companyId, status: { in: ["DISPATCHED", "IN_PROGRESS"] } } }),
    db.fuelTank.count({ where: { companyId, isActive: true } }),
    db.workOrder.count({ where: { companyId, status: { in: ["OPEN", "IN_PROGRESS"] } } }),
    db.maintenanceSchedule.count({ where: { companyId, status: "DUE" } }),
    db.supplier.count({ where: { companyId, isActive: true } }),
    db.purchaseOrder.count({ where: { companyId, status: { in: ["DRAFT", "SENT", "PARTIAL"] } } }),
    ...
    db.productionLine.count({ where: { companyId, isActive: true } }),
...
export async function getMonthlyTrends(companyId: string, months = 6) {
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    const [
      ...
      db.dispatchOrder.aggregate({ where: { companyId, createdAt: { gte: start, lte: end } }, _sum: { totalAmount: true } }),
    ...
    result.push({
      ...
      batches,
      dispatchOrders: dispatchOrders._sum.totalAmount || 0,
...
export async function getOperationalMetrics(companyId: string) {
  const [
    ...
    db.stockBalance.groupBy({ by: ["warehouseId"], where: { companyId }, _sum: { quantity: true } }),
    db.tripOrder.groupBy({ by: ["status"], where: { companyId }, _count: { id: true } }),
    db.fuelIssue.groupBy({ by: ["vehicleId"], where: { companyId }, _sum: { quantity: true }, take: 10 }),
  ...
  return {
    ...
    fuelConsumptionByVehicle: fuelConsumptionByVehicle.map((f) => ({ name: vehicleMap[f.vehicleId || ""] || f.vehicleId, value: f._sum.quantity || 0 })),
